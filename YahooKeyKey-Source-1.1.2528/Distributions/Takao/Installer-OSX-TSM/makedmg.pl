...
$ENV{WRAPPER_NAME} = "Yahoo! KeyKey.bundle";
...
@hdiutilOutput = `hdiutil create -sectors $imageSize -volname "Yahoo! KeyKey for Mac OS X Tiger.localized" -attach -fs HFS+ "$build_products_directory/Yahoo! KeyKey.dmg"`;
...
system("rm -f \"$build_products_directory/YahooKeyKey-OSX-Tiger-" . $version . ".dmg\"");
mySystem("hdiutil convert \"$build_products_directory/Yahoo! KeyKey.dmg\" -format UDZO -imagekey zlib-level=9 -o \"$build_products_directory/YahooKeyKey-OSX-Tiger-" .  $version . ".dmg\"");
system("rm -f \"$build_products_directory/Yahoo! KeyKey.dmg\"");
print "-----Done building.\n";
system("rm openUp");
