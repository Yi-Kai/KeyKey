...
if ARGV.size < 5
  $stderr.puts "usage: lm-tonemarker lm-file bpmf-cin tsi-src output-lm output-alias_chars"
  exit 1
end
...
$stderr.puts "reading tsi.src"
b = Benchmark.measure do
file_tsi_src = File.open(tsi_src, "r")
while line = file_tsi_src.gets
  d = line.chomp.split(/\s+/)
  key = d.shift  
  keys = key.unpack("U*").map {|k| [k].pack("U") }
...
$stderr.puts "tsi.src read: #{b.real} secs"
